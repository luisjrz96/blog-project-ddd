...
class UpdateAuthorProfileHandlerTest {

  @Mock private AuthorProfileRepository repository;
  @Mock private UserProvider userProvider;
  @Mock private TransactionalExecutor tx;
  @InjectMocks private UpdateAuthorProfileHandler handler;

  @Test
  void handle_shouldUpdateAuthorProfile_whenAuthorProfileAlreadyExist() {
  ...
  @Test
  void handle_shouldThrowException_whenAuthorProfileDoesNotExist() {
    Actor user = new Actor("41dece32-24b0-4ce0-95e9-9eb77d113400", Set.of(Role.ROLE_AUTHOR.name()));
    AuthorProfile authorProfileInDb = mock(AuthorProfile.class);
    AuthorId authorId = new AuthorId(UUID.fromString(user.userId()));
    var cmd =
    ...
    assertTrue(ex.getMessage().contains("first create your profile"));
