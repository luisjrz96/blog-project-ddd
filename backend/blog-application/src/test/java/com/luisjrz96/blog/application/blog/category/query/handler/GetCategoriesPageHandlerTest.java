...
class GetCategoriesPageHandlerTest {

  @Test
  void handle_shouldReturnCategoryPageWithStatusFromReader() {
    // given
    ...
    when(reader.getPageWithStatus(CategoryStatus.ACTIVE, pageRequest)).thenReturn(expectedPage);
    ...
    verify(reader, times(1)).getPageWithStatus(CategoryStatus.ACTIVE, pageRequest);
    verifyNoMoreInteractions(reader);
  }

  @Test
  void handle_shouldReturnCategoryPageFromReader() {
    // given
    CategoryViewReader reader = mock(CategoryViewReader.class);
    GetCategoriesPageHandler handler = new GetCategoriesPageHandler(reader);

    PageRequest pageRequest = PageRequest.of(0, 2);
    CategoriesPageQuery query = new CategoriesPageQuery(null, pageRequest);
